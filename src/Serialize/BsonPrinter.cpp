...
void BsonPrinter::closeMap()
{
    ...
    {
        throw std::runtime_error("ThorsAnvil::Serialize::BsonPrinter: Invalid call to closeMap(): Currently not in a map");
    }
    state.pop_back();
    output << PrefixMapClose(config.characteristics, state.size(), state.back()) << "}";
}
HEADER_ONLY_INCLUDE
void BsonPrinter::openArray(std::size_t)
{
...
void BsonPrinter::closeArray()
{
    ...
    {
        throw std::runtime_error("ThorsAnvil::Serialize::BsonPrinter: Invalid call to closeArray(): Currently not in an array");
    }
    state.pop_back();
...
void BsonPrinter::addKey(std::string const& key)
{
    if (state.back().second != TraitType::Map)
    {
        throw std::runtime_error("ThorsAnvil::Serialize::BsonPrinter: Invalid call to addKey(): Currently not in a map");
    }
...
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(short value)                 {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(int value)                   {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(long value)                  {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(long long value)             {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}

HEADER_ONLY_INCLUDE void BsonPrinter::addValue(unsigned short value)        {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(unsigned int value)          {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(unsigned long value)         {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(unsigned long long value)    {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}

HEADER_ONLY_INCLUDE void BsonPrinter::addValue(float value)                 {output << PrefixValue(config.characteristics, state.size(), state.back()) << FormatDouble<float>(value);}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(double value)                {output << PrefixValue(config.characteristics, state.size(), state.back()) << FormatDouble<double>(value);}
HEADER_ONLY_INCLUDE void BsonPrinter::addValue(long double value)           {output << PrefixValue(config.characteristics, state.size(), state.back()) << FormatDouble<long double>(value);}

HEADER_ONLY_INCLUDE void BsonPrinter::addValue(bool value)                  {output << PrefixValue(config.characteristics, state.size(), state.back()) << std::boolalpha << value;}
...
HEADER_ONLY_INCLUDE
void BsonPrinter::addRawValue(std::string const& value) {output << PrefixValue(config.characteristics, state.size(), state.back()) << value;}

HEADER_ONLY_INCLUDE
void BsonPrinter::addNull()                             {output << PrefixValue(config.characteristics, state.size(), state.back()) << "null";}
